...
/* ---------- imprimir a agenda de uma congregacao ---------- */
function montarImpressaoCongr(){
 const lateral=document.querySelector('.legend')||document.querySelector('.sidebar');
 if(!lateral){setTimeout(montarImpressaoCongr,700);return}
 if(typeof imprimirCongregacao!=='function')return;

 let cx=document.getElementById('imprCongr');
 if(!cx){
  const t=document.createElement('h4');
  t.id='imprCongrTitulo';
  t.style.marginTop='16px';
  t.textContent='IMPRIMIR PARA O MURAL';
 ...
 const fixo=(perfil&&perfil.papel==='secretario_congr'&&perfil.congregacao)
            ?perfil.congregacao:'';
 const doSetor=(perfil&&perfil.papel==='secretario'&&perfil.setor)?perfil.setor:'';

 let html='';
 if(fixo){
  html='<p class="ic-dica">Programa\u00e7\u00e3o de <b>'+fixo+'</b>, mais os eventos do setor e do campo.</p>'+
       '<button type="button" class="chip on ic-bt" data-c="'+fixo+'">Baixar PDF do mural</button>';
 }else if(doSetor){
  html='<p class="ic-dica">Escolha a congrega\u00e7\u00e3o do <b>'+doSetor+'</b>:</p>'+
       '<select id="icSel" class="select"><option value="">Todo o '+doSetor+'</option>'+
       CONGREGACOES.filter(function(c){return c.setor===doSetor})
        .map(function(c){return '<option>'+c.nome+'</option>'}).join('')+
       '</select>'+
       '<button type="button" class="chip on ic-bt" data-c="">Baixar PDF do mural</button>';
 }else{
  const ops=CONGREGACOES.map(function(c){
   return '<option value="'+c.nome+'">'+c.nome+'</option>';
  }).join('');
  if(!ops)return;
  html='<p class="ic-dica">Escolha a congrega\u00e7\u00e3o:</p>'+
       '<select id="icSel" class="select"><option value="">Escolha...</option>'+ops+'</select>'+
       '<button type="button" class="chip on ic-bt" data-c="">Baixar PDF do mural</button>';
 }

 if(cx.innerHTML===html)return;
 cx.innerHTML=html;

 const bt=cx.querySelector('.ic-bt');
 if(!bt)return;
 bt.onclick=function(){
  let congr=bt.getAttribute('data-c')||'';
  let setor='';
  const sel=document.getElementById('icSel');
  if(sel)congr=sel.value;
  if(congr){
   const achou=CONGREGACOES.filter(function(c){return c.nome===congr})[0];
   setor=achou?achou.setor:'';
  }else if(doSetor){
   setor=doSetor;
  }else if(fixo){
   congr=fixo;
   const achou=CONGREGACOES.filter(function(c){return c.nome===fixo})[0];
   setor=achou?achou.setor:'';
  }
  if(!congr&&!setor){aviso('Escolha a congrega\u00e7\u00e3o antes de baixar.');return}
  imprimirCongregacao(congr,setor);
 };
}
...
function marcarVersao(){
 ...
 p.id='versaoAgenda';
 p.textContent='Agenda v111 \u00b7 14.09.2026';
 alvo.appendChild(p);
}
...
setTimeout(montarImpressaoCongr,1200);
