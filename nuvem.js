...
function atualizarBarraLogin(){
 ...
 if(usuario){
  ...
  const eng=(ehSecretaria())
   ...
   :'';
  const funcao=textoFuncao();
  const iguais=(semAcento(nome).trim()===semAcento(funcao).trim());
  box.innerHTML='<div class="bl-quem"><span class="bl-nome">'+nome+'</span>'+
   (iguais?'':'<span class="bl-func">'+funcao+'</span>')+'</div>'+voltar+sino+chave+eng+
...
const TINTA_AMBITO={
 'Campo':{fundo:'#fadedd',texto:'#8c2f27'},
 'Setor':{fundo:'#d5efec',texto:'#0a5d57'}
};
TINTA_AMBITO[AMBITO_NOVO]={fundo:'#ebe2fa',texto:'#4a2a7d'};

function fundoDoDia(ambitos){
 if(!ambitos.length)return '';
 if(ambitos.length===1){
  const c=TINTA_AMBITO[ambitos[0]]||TINTA_AMBITO['Setor'];
  return 'background:'+c.fundo+';color:'+c.texto+';';
 }
 const fatia=Math.round(100/ambitos.length);
 const partes=[];
 ambitos.forEach(function(a,i){
  const c=(TINTA_AMBITO[a]||TINTA_AMBITO['Setor']).fundo;
  partes.push(c+' '+(i*fatia)+'%');
  partes.push(c+' '+((i+1)*fatia)+'%');
 });
 const t=(TINTA_AMBITO[ambitos[0]]||TINTA_AMBITO['Setor']).texto;
 return 'background:linear-gradient(135deg,'+partes.join(',')+');color:'+t+';';
}
 ...
 for(let m=0;m<12;m++){
  ...
  for(let dia=1;dia<=ultimo.getDate();dia++){
   ...
   evs.forEach(function(e){
    const a=(e.scope||'Setor');
    if(ambitos.indexOf(a)<0)ambitos.push(a);
   });
   const ordem=['Campo','Setor',AMBITO_NOVO];
   ambitos.sort(function(x,y){return ordem.indexOf(x)-ordem.indexOf(y)});
   const fundo=fundoDoDia(ambitos);
   const cls='va-tem'+(temPend?' va-pend':'');
   const titulos=evs.map(function(e){
    return e.time+' '+e.title+' ('+(e.scope||'Setor')+')';
   }).join(' | ');
   html+='<i class="'+cls+'" data-dia="'+iso+'" style="'+fundo+'" title="'+
         String(titulos).replace(/"/g,'&quot;')+'">'+dia+'</i>';
  }
  html+='</div></div>';
 }
 html+='</div><p class="va-legenda">'+
  '<span class="va-p" style="background:'+TINTA_AMBITO['Campo'].fundo+'"></span> Campo &nbsp; '+
  '<span class="va-p" style="background:'+TINTA_AMBITO['Setor'].fundo+'"></span> Setor &nbsp; '+
  '<span class="va-p" style="background:'+TINTA_AMBITO[AMBITO_NOVO].fundo+'"></span> Congrega\u00e7\u00e3o &nbsp; '+
  '<span class="va-p p3"></span> aguardando aprova\u00e7\u00e3o &nbsp;&nbsp;'+
  'Dias com mais de um \u00e2mbito aparecem divididos. Clique num dia para abrir o m\u00eas.</p></div>';
...
function marcarVersao(){
 ...
 p.id='versaoAgenda';
 p.textContent='Agenda v130 \u00b7 15.09.2026';
 alvo.appendChild(p);
}
...
function prepararSequencia(m,id){
 ...
 if(ev&&ev.grupo){
  ...
  const irmaos=events.filter(function(e){return e.grupo===ev.grupo})
                     .sort(function(x,y){return String(x.date).localeCompare(String(y.date))});
  const podeMexer2=podeMexer(id);
  let linhas='';
  irmaos.forEach(function(e){
   const d=new Date(String(e.date)+'T12:00');
   const rotulo=isNaN(d)?e.date:d.toLocaleDateString('pt-BR',{weekday:'short',day:'2-digit',month:'2-digit'});
   const aqui=(String(e.id)===String(id));
   linhas+='<div class="seq-dia'+(aqui?' atual':'')+'" data-ev="'+e.id+'">'+
    '<span class="sd-data">'+rotulo.replace('.','')+'</span>'+
    '<input type="time" class="sd-hora" value="'+(e.time||'')+'"'+(podeMexer2?'':' disabled')+'>'+
    (podeMexer2?'<button type="button" class="sd-x" title="Remover este dia">&times;</button>':'')+
    '</div>';
  });
  box.innerHTML='<label>Per\u00edodo \u2014 '+irmaos.length+' dia(s)</label>'+
   '<div class="seq-dias">'+linhas+'</div>'+
   '<span class="seq-dica">Ajuste o hor\u00e1rio de cada dia. O \u00d7 remove s\u00f3 aquele dia.</span>'+
   (podeMexer2?'<button type="button" class="chip seq-apagar">Excluir o per\u00edodo inteiro</button>':'');
 }else if(!id&&usuario){
 ...
 box.querySelectorAll('.sd-hora').forEach(function(inp){
  inp.addEventListener('change',function(){
   const alvo=inp.closest('.seq-dia').getAttribute('data-ev');
   const e2=events.filter(function(x){return String(x.id)===String(alvo)})[0];
   if(!e2)return;
   e2.time=inp.value;
   gravarLocal();
   if(typeof save==='function')save();
   if(typeof render==='function')render();
   aviso('Hor\u00e1rio do dia atualizado.');
  });
 });
 box.querySelectorAll('.sd-x').forEach(function(bx){
  bx.onclick=function(){
   const alvo=bx.closest('.seq-dia').getAttribute('data-ev');
   const e2=events.filter(function(x){return String(x.id)===String(alvo)})[0];
   if(!e2)return;
   const d=new Date(String(e2.date)+'T12:00');
   const rot=isNaN(d)?e2.date:d.toLocaleDateString('pt-BR');
   if(!confirm('Remover apenas o dia '+rot+' deste evento?'))return;
   excluirDiaDaSerie(alvo);
  };
 });
...
function aplicarSequencia(){
 ...
 // so age sobre o evento que o formulario acabou de criar; sem isso, nao faz nada
...
/* ---------- excluir dia unico ou o periodo inteiro ---------- */
function ligarExclusaoSerie(){
 if(typeof deleteEvent!=='function'||deleteEvent._comSerie)return;
 const _orig=deleteEvent;
 const novo=function(){
  const id=(typeof editingId!=='undefined')?editingId:null;
  if(!id)return _orig.apply(null,arguments);
  const ev=events.filter(function(e){return String(e.id)===String(id)})[0];
  if(!ev||!ev.grupo)return _orig.apply(null,arguments);
  const irmaos=events.filter(function(e){return e.grupo===ev.grupo});
  if(irmaos.length<2)return _orig.apply(null,arguments);
  const d=new Date(String(ev.date)+'T12:00');
  const rot=isNaN(d)?ev.date:d.toLocaleDateString('pt-BR');
  const todos=confirm('Este evento tem '+irmaos.length+' dias.\n\n'+
   'OK = excluir os '+irmaos.length+' dias\n'+
   'Cancelar = excluir apenas o dia '+rot);
  if(todos)excluirSequencia(ev.grupo);
  else excluirDiaDaSerie(id);
 };
 novo._comSerie=true;
 deleteEvent=novo;
}

async function excluirDiaDaSerie(id){
 if(!sb)return;
 registrarIntencao(id);
 const r=await sb.from('eventos').delete().eq('id',String(id)).select('id');
 if(r.error){aviso('N\u00e3o foi poss\u00edvel remover o dia: '+r.error.message);return}
 if(!r.data||!r.data.length){
  const res=await tratarExclusaoSemEfeito(id,'remover este dia');
  if(res==='recusado'){mostrarRelatorioExcl();return}
 }
 marcarExcluido(id);
 for(let i=events.length-1;i>=0;i--){
  if(String(events[i].id)===String(id))events.splice(i,1);
 ...
 delete ultimoEstado[String(id)];
 // recontar os dias restantes da serie
 const ev=null;
 gravarLocal();
 fecharModal(acharModal());
 render();
 atualizarSino();
 aviso('Dia removido do evento.');
}
...
function aplicarDeptoPendente(){
 ...
 // eventos novos ja recebem o departamento no proprio formulario.
 // aqui so tratamos edicao de evento existente, com alvo conhecido.
 const alvoDep=_deptoPendente.alvo;
 if(!alvoDep){_deptoPendente=null;return}
 let idx=-1;
 for(let i=0;i<events.length;i++){
...
setTimeout(ligarExclusaoSerie,1100);
